...
int compare(const void *a, const void * b){
    ...
    if(first.score < second.score)    // CHECK!!!!
        return 1;

    else if(first.score > second.score)    // CHECK!!!!
        return -1;
...
char* solution(char* names[], int names_len, int scores[], int scores_len, int K) {
    ...
    for(int i = 0; i < len; i++){
        strcpy(students[i].name, names[i]);    //kioeom    // CHECK!!!!
        students[i].score=scores[i];    //kioeom    // CHECK!!!!
    }
    qsort(students, len, sizeof(Student), compare);
    return students[K-1].name;    // CHECK!!!!  
}
