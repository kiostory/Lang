...
int solution(char* expression) {
    int exp_index = func_b(expression);    //kioeom
    Pair numbers = func_c(expression,exp_index);    //kioeom
    int result = func_a(numbers.first_num, numbers.second_num, expression[exp_index]);    //kioeom
    return result;
}
